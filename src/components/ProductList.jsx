...
const ProductList = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get('https://campusbackend-fk2p.onrender.com/api/products');
              ...
              {productsByCategory[category].map((product) => (
                <div className="product-card" key={product._id}>
                  <Link to={`/product/${product._id}`}>
                    <img src={`https://campusbackend-fk2p.onrender.com${product.images[0]}`} alt={product.name} className="product-image" />
                    <h3>{product.name}</h3>
